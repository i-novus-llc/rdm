    ...
    public void init() {
        ...
        try {
            ...
            for (ClassifierAttribute attr : attrs) {
                ...
                if (attr.isRequired())
                    writeValidation(AttributeValidationType.REQUIRED.name(), "true");
                if (attr.getRegex() != null && !attr.getRegex().isBlank())
                    writeValidation(AttributeValidationType.REG_EXP.name(), attr.getRegex());
                if (attr.getLength() != null && attr.getLength() > 0)
                    writeValidation(AttributeValidationType.PLAIN_SIZE.name(), attr.getLength().toString());
                if (attr.getIntStartRange() != null || attr.getIntEndRange() != null)
                    writeValidation(AttributeValidationType.INT_RANGE.name(), (attr.getIntStartRange() == null ? "" : attr.getIntStartRange().toString()) + ";" + (attr.getIntEndRange() == null ? "" : attr.getIntEndRange().toString()));
                else if (attr.getDecimalStartRange() != null || attr.getDecimalEndRange() != null)
                    writeValidation(AttributeValidationType.FLOAT_RANGE.name(), (attr.getDecimalStartRange() == null ? "" : attr.getDecimalStartRange().toString()) + ";" + (attr.getDecimalEndRange() == null ? "" : attr.getDecimalEndRange().toString()));
                    ...
                    writeValidation(AttributeValidationType.DATE_RANGE.name(), start.format(RDM_DATE_FORMAT) + ";" + end.format(RDM_DATE_FORMAT));
                }
                writer.writeEndElement();
            }
            writer.writeEndElement();
