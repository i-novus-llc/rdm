...
public class PageIteratorTest {
    ...
    public void testIteration() {
        Function<TestCriteria, Page<String>> pageSource = (criteria) -> {
            ...
            List<String> content = allContent.subList(Math.min(criteria.getPageNumber()*criteria.getPageSize(),allContent.size()), Math.min((criteria.getPageNumber() + 1)*criteria.getPageSize(), allContent.size()));
            return new PageImpl<String>(content, criteria, total);
        };
        ...
        for (int i = 0; i<3; i++) {
            ...
            String displayContent = page.getContent().stream().collect(Collectors.joining(","));
