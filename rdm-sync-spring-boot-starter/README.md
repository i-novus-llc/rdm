...
## Возможность пульнуть данные в RDM
- Если в вашем приложении есть какая - то сущность, которая отражена в RDM -- вы можете пульнуть ее туда.  
Сущности, которые вы пуляете в RDM должны реализовывать интерфейс `Serializable`. То же самое применяется для всех полей сущности рекурсивно. Вы можете передать либо экземпляр какой - то конкретной сущности, либо `Map`. В первом случае все поля сущности (вплоть до `Object`-а) будут сложены в `Map` (нерелевантные поля будут отфильтрованы на стороне RDM).  
...
Базовый интерфейс для данной операции отражен в `ru.inovus.ms.rdm.sync.service.change_data.ChangeDataClient`.  
...
Результаты и синхронной, и асинхронной операций можно перехватить через `ru.inovus.ms.rdm.sync.service.change_data.ChangeDataRequestCallback`. По - умолчанию она просто логгирует в консоль.  
