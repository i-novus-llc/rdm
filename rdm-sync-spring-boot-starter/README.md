...
Вы также можете настроить создание таблиц через `rdm-mapping.xml`. Таблицы создадутся уже не по структуре справочника из RDM, а по вашему XML-файлу. Для этого просто добавьте в ресурсы этот файл, укажите код справочника в настройке `rdm_sync.auto_create.ref_book_codes` и все автоматом создастся.  
`rdm-mapping.xml` имеет приоритет выше, чем `rdm_sync.auto_create.ref_book_codes`. То есть если вы указали справочник и в `rdm-mapping.xml` и в `rdm_sync.auto_create.ref_book_codes`, в настройке этот справочник просто проигнорируется.  
