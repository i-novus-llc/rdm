...
То есть вы указываете настройки `rdm_sync.auto_create.ref_book_codes` и `rdm_sync.auto_create.schema`. Поднимаете локально свое приложение. 
Стартер скачает справочники из РДМ и сам сгенерит таблички. Но вам, скажем, не нужна половина полей из RDM. Вы можете прямо в браузере сходить по адресу `localhost:8080/api/rdm/xml-fm?code=REF_BOOK_CODE1&code=REF_BOOK_CODE2&code=REF_BOOK_3&...`. 
...
Вы также можете сходить по адресу `localhost:8080/api/rdm/xml-fm?code=all`. Так вы скачаете файл, в котором перечислен маппинг для каждого справочника, ведущегося в вашем приложении.