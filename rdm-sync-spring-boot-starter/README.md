...
Вторая задается в формате `{'REF_BOOK_CODE1': 'CLIENT_TABLE1', 'REF_BOOK_CODE2': 'CLIENT_TABLE2', ...}`. Ключом идет код справочника, под который необходимо автоматически создать клиентскую таблицу. Значением соответственно идет название этой таблицы.  
Вы также можете настроить создание таблиц через `rdm-mapping.xml`. Таблицы создадутся уже не по структуре справочника из RDM, а по вашему XML-файлу. Для этого просто добавьте в ресурсы этот файл и все автоматом создастся.  
