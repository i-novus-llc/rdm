...
Диаграмма переходов состояний, при условии, что запись не модифицируется во время пребывания ее в состоянии `PENDING` или `ERROR`, выглядит так:  
`SYNCHED`←`ERROR`  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;↑&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;⤨  
`PENDING`←`DIRTY`  
Со временем запись из состояния `ERROR` должна перейти в запись с состоянием `SYNCED` (будет вечный ретрай)