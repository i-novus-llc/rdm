...
**Важно:**
liquibase rdm-sync запускается ПОСЛЕ общего liquibase, сконфигурированного по умолчанию.
...
Этот способ устаревший и не рекомендуется для использования, в будущем он будем выключен.
...
В случае изменения маппинга меняем в файле соответвующий элемент refbook и увеличиваем mapping-version на 1.
...
Маппинг должен соответствовать тому, что есть в НСИ и в клиентском приложении.
То есть в маппинге не должно быть полей, несуществующих в НСИ и в локальной таблице.
...
- Дробный из НСИ можно мапить в "numeric", "decimal", "varchar", "text", "character varying".
...
- `code` - колонка с любым типом, совместимым с типом первичного ключа справочника НСИ. В эту колонку будет копироваться значение первичного ключа справочника из НСИ. Указывается в колонке rdm_sync.version.unique_sys_field
...
Их количество и название не обязательно должны совпадать.
...
1. Задать значение свойств `rdm_sync.publish.listener.enable`, `spring.activemq.broker-url`.  
